...
+ 代理需要支持 stream 方式的返回
+ HTTP `target` 配置了 `http_proxy` 时，请求会经 `http_proxy` 转发；HTTPS `target` 配置了 `https_proxy` 时，请求会先通过代理建立 CONNECT 隧道，再与 `target` 建立 TLS 连接
+ `http_proxy` 和 `https_proxy` 的值都可使用 `http://` 或 `https://` 代理地址；未配置与 target 协议匹配的代理时直接连接 target
...
服务启动后，当访问 `/v1/models` 时，服务会根据配置中的 `target`，通过 HTTP GET 调用 `{target}/v1/models`。请求使用与 target 协议匹配的 `http_proxy` 或 `https_proxy`，并添加 `ext_headers`，请求会返回如下 json 内容：
```json
