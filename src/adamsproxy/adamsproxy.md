...
+ 接收的请求为 `openai` 规范的 `/chat/completions` 或者 `/messages` 请求
+ 接收请求后，会调整请求，并根据请求中的 `model` 字段，匹配配置中的 `names`，转发到不同的 `target` 上
+ 代理需要支持 stream 方式的返回
...
```
+ 如果服务启动时，配置文件的路径没有通过命令行参数传入，使用 `$HOME/.config/adamsproxy.json` 作为配置路径
